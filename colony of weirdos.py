...
from sklearn.metrics import confusion_matrix, f1_score, accuracy_score, roc_curve, roc_auc_score,recall_score
...
url = 'https://storage.googleapis.com/kaggle-data-sets/3125926/5392837/bundle/archive.zip?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=gcp-kaggle-com%40kaggle-161607.iam.gserviceaccount.com%2F20230528%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20230528T222244Z&X-Goog-Expires=259200&X-Goog-SignedHeaders=host&X-Goog-Signature='

response = r.get(url)
dataset_res = ZipFile(BytesIO(response.content)).open('ds_salaries.csv')
data_set = pd.read_csv(dataset_res)
print(data_set)
...
print(data_set.dropna().shape)
print(data_set.isnull().sum())
print(data_set.info())
dataset = data_set.drop(["work_year"], axis = 1)
dataset = data_set.drop(["remote_ratio"], axis = 1)
MI= dataset[data_set.experience_level == "MI"]
print(MI.head(3))
EN= dataset[data_set.experience_level == "EN"]
print(EN.head(53))
print(dataset.head(3))
data_set.experience_level = [1 if i == "MI" else 0 for i in data_set.experience_level]
x = data_set.drop(["experience_level"], axis = 1)
y = data_set.experience_level.values
...
# To read dataset (.csv file) from Data.world site, we can use the next program:
print("---------------------------------------------")
...
print("---------------------------------------------")
print(Y)
#To plot these data as a scattering chart  , we should select
# numerical columns. We should select two numerical columns. One of them is
# input such as the third column X[:,2]. The second one is the output. Here, Y is
# the output and one column. Thus, we will add the next lines for our program:
ax=plt.gca()
ax.plot(X[:,2],Y,'bo')
ax.set_xlabel("employment type ")
ax.set_ylabel("company size")
plt.show()
#We depend on matplotlib library to plot the relation between two variables.
# Thus, we should install it first by using:
# import matplotlib.pyplot as plt

# If we want to plot the histogram of the output, we can add the next lines:
ax2=plt.gca()
ax2.hist(Y)
plt.show()
# In each column, how many element without repeating, their types, and
# what are these elements?
print(data_set.nunique())
print("----------------------------")
print(data_set.dtypes)
# data.Duration.unique() is used to show the
# elements in column title « Duration » without repeating.
print(data_set.work_year.unique())
# There are two types of empty cell. One of them contains the value “NaN”
# means Non-value. The other type does not contain any value or empty.
# To test there is empty cells within the data, we will compare the dimensions
# of the data before and after deleting the rows that contains empty cells as follows.
print(data_set.shape)
print(data_set.dropna().shape)
print(data_set.isnull().sum())
# To delete one row. For example, row of index = 0
data = data_set.drop(0)
# To delete more than one row:
data = data_set.drop([0, 15, 20])
# To delete range of rows:
data = data_set.drop(range(10,20))
# Another example of pre-processing is the renaming the columns-titles in the dataset.
data_new=data_set.rename(columns={"work_year":"work_year  "})
print(data_new.head(0))
# To compare between the old and new results, you can use:
print(data_set.head(0))
print(data_new.head(0))
# There are more than one way to deal with empty cells:
# Way 1: Remove all the dataset rows that contain empty cell.
data = data_set.dropna()
# Or, Remove all the dataset rows based on empty cells in some column.
data_set.dropna(subset=['work_year'], inplace = True)
# Way 2:
# Replace the NaN by some value.
data_set.work_year = data.work_year.fillna(5)
data = data_set.fillna(15)
# Way 3:For some column, Replace the ”NaN” value by:  mean = the average value (the sum of all values divided by
# number of values).  median = the value in the middle, after you have sorted all values
# ascending.  mode = the value that appears most frequently.
X_mean = data_set["work_year"].mean()
X_median = data_set["salary_in_usd"].median()
X_mode = data_set["salary"].mode()[0]
data["work_year"] = data_set["work_year"].fillna(X_mean)
data["salary_in_usd"] = data_set["salary_in_usd"].fillna(X_median)
data["salary"] = data_set["salary"].fillna(X_mode)
print(X_mean, X_median, X_mode)
# Data of Wrong Format
# Cells with data of wrong format can make it difficult, or even impossible,
# to analyse data. To fix it, you have two options: remove the rows, or
# convert all cells in the columns into
print(data['work_year'][0])
data['work_year'] = pd.to_datetime(data['work_year'])
print("------------------")
print(data['work_year'][0])
print(data['work_year'][0])
data['work_year'] = pd.to_numeric(data['work_year'])
print("------------------")
print(data['work_year'][0])
# You can depend on numpy library for data conversion such as:
print(data['work_year'][0])
data['work_year'] = np.float32(data['work_year'])
print("------------------")
print(data['work_year'][0])
# "Wrong data" does not have to be "empty cells" or "wrong format", it can
# just be wrong, like the age of someone is registered as “199" , “-50” , or
# “40” instead of “35”. We can fix this wrong data by replacing the values
# or removing the rows.
# One of the way used to follow the wrong data in the dataset is the using of charts.
X = [i for i in range(data.shape[0])]
plt.plot(X,data['work_year'])
plt.show()
plt.plot(X,data['salary_in_usd'])
plt.show()
  ...
  "cells": [
    ...
    },
    {
      ...
      "source": [
        ...
        "from io import BytesIO\n",
        "import requests as r\n",
        "url = 'https://storage.googleapis.com/kaggle-data-sets/3125926/5392837/bundle/archive.zip?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=gcp-kaggle-com%40kaggle-161607.iam.gserviceaccount.com%2F20230527%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20230527T184042Z&X-Goog-Expires=259200&X-Goog-SignedHeaders=host&X-Goog-Signature='\n",
        "response = r.get(url)\n",
        "dataset_res = ZipFile(BytesIO(response.content)).open('ds_salaries.csv')\n",
        "df = pd.read_csv(dataset_res)\n"
      ],
      "metadata": {
        "cellView": "code",
        "id": "IjglnAHyekze"
    ...
    },
    {
      ...
      "metadata": {
        "id": "xMfz98mxChwh",
        "outputId": "f7840021-64f6-4904-887b-dc219255bf8d",
        "colab": {
      ...
      "outputs": [
        ...
        },
        {
          ...
          "traceback": [
            ...
            "\u001b[0;32m<ipython-input-28-201e5b063a80>\u001b[0m in \u001b[0;36m<cell line: 15>\u001b[0;34m()\u001b[0m\n\u001b[1;32m     13\u001b[0m         \u001b[0mprint\u001b[0m\u001b[0;34m(\u001b[0m\u001b[0mdf\u001b[0m\u001b[0;34m[\u001b[0m\u001b[0mcolumn\u001b[0m\u001b[0;34m]\u001b[0m\u001b[0;34m.\u001b[0m\u001b[0munique\u001b[0m\u001b[0;34m(\u001b[0m\u001b[0;34m)\u001b[0m\u001b[0;34m)\u001b[0m\u001b[0;34m\u001b[0m\u001b[0;34m\u001b[0m\u001b[0m\n\u001b[1;32m     14\u001b[0m \u001b[0;34m\u001b[0m\u001b[0m\n\u001b[0;32m---> 15\u001b[0;31m \u001b[0mfit\u001b[0m \u001b[0;34m=\u001b[0m \u001b[0mbestfeatures\u001b[0m\u001b[0;34m.\u001b[0m\u001b[0mfit\u001b[0m\u001b[0;34m(\u001b[0m\u001b[0mX\u001b[0m\u001b[0;34m,\u001b[0m\u001b[0my\u001b[0m\u001b[0;34m)\u001b[0m\u001b[0;34m\u001b[0m\u001b[0;34m\u001b[0m\u001b[0m\n\u001b[0m\u001b[1;32m     16\u001b[0m \u001b[0mdfscores\u001b[0m \u001b[0;34m=\u001b[0m \u001b[0mpd\u001b[0m\u001b[0;34m.\u001b[0m\u001b[0mDataFrame\u001b[0m\u001b[0;34m(\u001b[0m\u001b[0mfit\u001b[0m\u001b[0;34m.\u001b[0m\u001b[0mscores_\u001b[0m\u001b[0;34m)\u001b[0m\u001b[0;34m\u001b[0m\u001b[0;34m\u001b[0m\u001b[0m\n\u001b[1;32m     17\u001b[0m \u001b[0mdfcolumns\u001b[0m \u001b[0;34m=\u001b[0m \u001b[0mpd\u001b[0m\u001b[0;34m.\u001b[0m\u001b[0mDataFrame\u001b[0m\u001b[0;34m(\u001b[0m\u001b[0mX\u001b[0m\u001b[0;34m.\u001b[0m\u001b[0mcolumns\u001b[0m\u001b[0;34m)\u001b[0m\u001b[0;34m\u001b[0m\u001b[0;34m\u001b[0m\u001b[0m\n",
    ...
    },
    {
      ...
      "metadata": {
        "id": "d4B87key15dx",
        "outputId": "f384e7b0-aaa1-426d-8843-7d5f17edfc16",
        "colab": {
          "base_uri": "https://localhost:8080/"
        }
      },
    ...
    },
    {
      ...
      "source": [
        ...
        "iteration_number=0\n",
        "for index_train, index_test in model.split(data):\n",
      ...
      "metadata": {
        "id": "3sv7xnwc1urK",
        "outputId": "704416c6-8c90-4f81-cff3-59d2d9b451e6",
        "colab": {
          "base_uri": "https://localhost:8080/"
        }
      },
...
url = 'https://storage.googleapis.com/kaggle-data-sets/3125926/5392837/bundle/archive.zip?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=gcp-kaggle-com%40kaggle-161607.iam.gserviceaccount.com%2F20230528%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20230528T222857Z&X-Goog-Expires=259200&X-Goog-SignedHeaders=host&X-Goog-Signature='
response = r.get(url)
dataset_res = ZipFile(BytesIO(response.content)).open('ds_salaries.csv')
df = pd.read_csv(dataset_res)
...
df.corr()
df.columns
df2=df[['work_year', 'experience_level', 'employment_type', 'remote_ratio','company_size']]
df2.info()
df2['work_year'].value_counts()
df2['experience_level'].value_counts()
df2['employment_type'].value_counts()
df2['company_size'].value_counts()
df2.replace(2023,0, inplace= True)
df2.replace(2022,1, inplace= True)
df2.replace(2021,2, inplace= True)
df2.replace(2020,3, inplace= True)
df2.replace('Entry-level/Junior',0, inplace= True)
df2.replace('Mid-level/Intermediate',1, inplace= True)
df2.replace('Senior-level/Expert',2, inplace= True)
df2.replace('Executive-level/Director',3, inplace= True)
df2.replace('FT',0, inplace= True)
df2.replace('PT',1, inplace= True)
df2.replace('CT',2, inplace= True)
df2.replace('FL',3, inplace= True)
df2.replace('0',0, inplace= True)
df2.replace('50',1, inplace= True)
df2.replace('100',2, inplace= True)
df2.replace('S',0, inplace= True)
df2.replace('M',1, inplace= True)
df2.replace('L',2, inplace= True)
df2.corr()
plt.figure(figsize=(10,6))
sns.heatmap(df2.corr(),cmap ='RdYlGn', annot =True)
df['remote_ratio'].describe()
bins = [11,21,99,111,150]
x =[1,2,3,4]
df['offer']=pd.cut(df['remote_ratio'], bins=bins ,labels= x)
df['salary_in_usd'].max()
df['salary_in_usd'].idxmax()
df.iloc[3522]
...
fig.show()
df["experience_level"].value_counts()['Senior-level/Expert']/df.shape[0]
df["experience_level"].value_counts()['Mid-level/Intermediate']/df.shape[0]
df["experience_level"].value_counts()['Entry-level/Junior']/df.shape[0]
df["experience_level"].value_counts()['Executive-level/Director']/df.shape[0]
df["experience_level"].value_counts()/df.shape[0]
round(df["experience_level"].value_counts()/df.shape[0]*100,4)
round(df["experience_level"].value_counts()/df.shape[0]*100,4).plot.pie(autopct= '%1.1f%%')
...
hist_data = [entry_salary['salary_in_usd'], mid_salary['salary_in_usd'], senior_salary['salary_in_usd'],
...
df.columns
df.replace(2023,0, inplace= True)
df.replace(2022,1, inplace= True)
df.replace(2021,2, inplace= True)
df.replace(2020,3, inplace= True)
df.replace('Entry-level/Junior',0, inplace= True)
df.replace('Mid-level/Intermediate',1, inplace= True)
df.replace('Senior-level/Expert',2, inplace= True)
df.replace('Executive-level/Director',3, inplace= True)
df.replace('FT',0, inplace= True)
df.replace('PT',1, inplace= True)
df.replace('CT',2, inplace= True)
df.replace('FL',3, inplace= True)
df.replace('0',0, inplace= True)
df.replace('50',1, inplace= True)
df.replace('100',2, inplace= True)
df.replace('S',0, inplace= True)
df.replace('M',1, inplace= True)
df.replace('L',2, inplace= True)
cols = {
    'work_year'   : df.work_year.median(),
    'remote_ratio': round(df.remote_ratio.mean(),2)
}
cols
df=df.fillna(cols)
df.head()
...
df['work_year']=normalize('work_year')
df['salary_in_usd']=normalize('salary_in_usd')
df['remote_ratio']=normalize('remote_ratio')
df=df.dropna()
df.head()
...
df.describe()
data_copy = df.copy()
data_copy[['work_year', 'experience_level', 'employment_type', 'salary_in_usd']] = data_copy[['work_year', 'experience_level', 'employment_type', 'salary_in_usd']].replace(0,np.NaN)
data_copy['work_year'].fillna(data_copy['work_year'].median(), inplace = True)
data_copy['experience_level'].fillna(data_copy['experience_level'].mean(), inplace = True)
data_copy['employment_type'].fillna(data_copy['employment_type'].median(), inplace = True)
data_copy['salary_in_usd'].fillna(data_copy['salary_in_usd'].median(), inplace = True)
data_copy['remote_ratio'].fillna(data_copy['remote_ratio'].median(), inplace = True)
data_copy['company_size'].fillna(data_copy['company_size'].median(), inplace = True)
data_copy.isna().sum()
data_copy.describe()
